...
export default function BackgroundPage() {
  ...
  const getWorrySeedPosition = (seed: WorrySeed) => {
    const convertToPercent = (value: number, axis: "x" | "y") => {
      if (value <= 100) return Math.min(Math.max(value, 4), 96);
      ...
      return Math.min(Math.max((value / size) * 100, 4), 96);
    };
  ...
  return (
    ...
    >
      ...
      >
        {worrySeeds.map((seed) => {
          const position = getWorrySeedPosition(seed);
